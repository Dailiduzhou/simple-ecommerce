...
SELECT i.post_id,i.sort_order,m.* FROM post_images i JOIN media_assets m ON m.id=i.media_id
JOIN posts p ON p.id=i.post_id WHERE i.post_id=ANY(sqlc.arg(post_ids)::bigint[]) AND p.deleted_at IS NULL AND m.status='ready' ORDER BY i.post_id,i.sort_order;
-- name: LockPostImageAssets :many
SELECT m.* FROM media_assets m JOIN post_images i ON i.media_id=m.id WHERE i.post_id=$1 ORDER BY m.id FOR UPDATE OF m;
